...
`.github/workflows/mockops-release.yml`, which runs on every push to
`main`, determines whether a release is actually required (an all-docs
change is skipped — see
[Reference → Architecture Decisions](/reference/architecture-decisions)),
and only then builds and pushes:
...
Tags follow [Semantic Versioning](https://semver.org/): `latest` and
`<major>.<minor>.<patch>` — generated by `docker/metadata-action` in the
release job, after the Git tag's version has already been computed from
the PRs merged since the last release. See
[Reference → Architecture Decisions](/reference/architecture-decisions)
and `AGENTS.md`'s "Versioning & releases" section for the full pipeline.
